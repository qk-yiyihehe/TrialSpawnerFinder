...
安装时控制台会显示完整的环境准备和服务端解包输出，下载过程使用包含百分比、已下载大小和速度的单行进度。运行搜索时会先显示版本、种子、JDK、端口、范围、线程、自动分片和搜索模式；粗筛与总进度随后按预计候选总量双行同时推进，分片数不参与百分比，最后单独显示榜单复核进度。每条进度都包含速度和阶段 ETA；IDE或重定向环境无法固定光标时自动改为每10秒一条快照。Minecraft 和 Fabric 的普通日志写入 `.runtime\minecraft-版本\logs\latest.log`，调试日志写入同目录的 `debug.log`；每次启动都会将上一份日志按完整时间归档到 `logs\archive`。发行版根目录的 `logs\launcher-版本-时间.log` 另外保存每次 Java 启动的完整输出，便于定位 Fabric 初始化前的失败。
...
`server-port` 默认为 `25566`，用于避开正常游戏服务端常用的 `25565`；同机并行运行多个搜索时应给每个实例配置不同端口。`java-home` 可填写自定义 JDK 根目录，例如 `D:\Java\graalvm-jdk-25`；留空时安装器自动寻找或下载 GraalVM 25。修改 `java-home` 后建议重新运行一次 `setup.bat`，安装器会检查该 JDK 是否兼容所选引擎。
