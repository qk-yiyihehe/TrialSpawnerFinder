...
安装时控制台会显示完整的环境准备和服务端解包输出，下载过程使用包含百分比、已下载大小和速度的单行进度。运行搜索时会先显示版本、种子、JDK、端口、范围、线程、自动分片和搜索模式，随后显示粗筛、总进度与最终榜单复核进度；每条进度都包含速度和阶段 ETA。Minecraft 和 Fabric 的普通日志写入 `.runtime\minecraft-版本\logs\latest.log`，调试日志写入同目录的 `debug.log`；每次启动都会将上一份日志按完整时间归档到 `logs\archive`。发行版根目录的 `logs\launcher-版本-时间.log` 另外保存每次 Java 启动的完整输出，便于定位 Fabric 初始化前的失败。
