...
试炼刷怪笼的专用配置统一使用 `trial-` 前缀。`trial-cluster-radius-blocks` 和 `trial-area-shape` 控制每个结果的统计范围；`trial-min-structures` 和 `trial-min-spawners` 是最低密室数与刷怪笼数。旧的无前缀配置名仍可读取，但新配置应使用带前缀名称。
